    ...
    public class MqttMessageBase
    {
        ...
        public byte[] Serialize(SerializeMode mode)
        {
            if (mode == SerializeMode.Json)
            {
                ...
                return Encoding.UTF8.GetBytes(ZipHelper.Compress(json));
            }
        ...
        public static MqttMessageBase DeserializeData(byte[] data, SerializeMode mode)
        {
            if (mode == SerializeMode.Json)
            {
                ...
                var datas = input;
                if (TaskCenterSetting.Current.ZipCompress)
                {
                    datas = ZipHelper.Decompress(input);
                }
                return JsonConvert.DeserializeObject<MqttMessageBase>(datas);
            }
