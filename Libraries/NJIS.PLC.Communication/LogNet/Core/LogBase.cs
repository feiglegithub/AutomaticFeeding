...
using NJIS.PLC.Communication.LogNet.Logs;
...
namespace NJIS.PLC.Communication.LogNet.Core
{
    ...
    public abstract class LogNetBase : IDisposable
    {
        ...
        public LogNetBase()
        {
            ...
            m_WaitForSave = new Queue<NjisMessageItem>();
        ...
        private NjisMessageDegree m_messageDegree = NjisMessageDegree.DEBUG; // 默认的存储规则
        private Queue<NjisMessageItem> m_WaitForSave; // 待存储数据的缓存
        private SimpleHybirdLock m_simpleHybirdLock; // 缓存列表的锁
        private int m_SaveStatus; // 存储状态
        ...
        private void OnBeforeSaveToFile(NjisEventArgs args)
        {
        ...
        {
            RecordMessage(NjisMessageDegree.DEBUG, keyWord, text);
        }
        ...
        {
            RecordMessage(NjisMessageDegree.INFO, keyWord, text);
        }
        ...
        {
            RecordMessage(NjisMessageDegree.WARN, keyWord, text);
        }
        ...
        {
            RecordMessage(NjisMessageDegree.ERROR, keyWord, text);
        }
        ...
        {
            RecordMessage(NjisMessageDegree.FATAL, keyWord, text);
        }
        ...
        {
            RecordMessage(NjisMessageDegree.FATAL, keyWord, LogNetManagment.GetSaveStringFromException(text, ex));
        }
        ...
        public void WriteDescrition(string description)
        {
            ...
            var count = 118 - CalculateStringOccupyLength(description);
            if (count >= 8)
            {
                var count_1 = (count - 8) / 2;
            ...
            ThreadPool.QueueUserWorkItem(ThreadPoolSaveText, stringBuilder.ToString());
        }
        ...
        {
            ThreadPool.QueueUserWorkItem(ThreadPoolSaveText, "\u0002" + Environment.NewLine);
        }
        ...
        private void WriteToFile(NjisMessageDegree degree, string keyWord, string text)
        {
            // 过滤事件
            if (degree <= m_messageDegree)
            {
                // 需要记录数据
                var item = GetNjisMessageItem(degree, keyWord, text);
                AddItemToCache(item);
            }
        }


        private void AddItemToCache(NjisMessageItem item)
        {
        ...
        private NjisMessageItem GetAndRemoveLogItem()
        {
            NjisMessageItem result = null;
        ...
        {
            // 获取需要存储的日志
            var current = GetAndRemoveLogItem();
            // 进入文件操作的锁
            m_fileSaveLock.Enter();


            // 获取要存储的文件名称
            var LogSaveFileName = GetFileSaveName();
            ...
            if (!string.IsNullOrEmpty(LogSaveFileName))
            {
                ...
                try
                {
                    ...
                    {
                        // 触发事件
                        OnBeforeSaveToFile(new NjisEventArgs {NjisMessage = current});
                ...
                catch (Exception ex)
                {
                    ...
                    AddItemToCache(new NjisMessageItem
                    {
                        Degree = NjisMessageDegree.FATAL,
        ...
        private string HslMessageFormate(NjisMessageItem hslMessage)
        {
            var stringBuilder = new StringBuilder("\u0002");
            stringBuilder.Append("[");
        ...
        private NjisMessageItem GetNjisMessageItem(NjisMessageDegree degree, string keyWord, string text)
        {
            return new NjisMessageItem
            {
                KeyWord = keyWord,
                Degree = degree,
                Text = text,
