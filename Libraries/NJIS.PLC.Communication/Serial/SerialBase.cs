...
namespace NJIS.PLC.Communication.Serial
{
    ...
    public class SerialBase : IDisposable
    {
        ...
        protected virtual OperateResult<byte[]> SPReceived(SerialPort serialPort, bool awaitData)
        {
            ...
            var start = DateTime.Now; // 开始时间，用于确认是否超时的信息
            while (true)
            {
                ...
                try
                {
                    ...
                    // 继续接收数据
                    var sp_receive = serialPort.Read(buffer, 0, buffer.Length);
        ...
        private readonly SerialPort SP_ReadData; // 串口交互的核心
