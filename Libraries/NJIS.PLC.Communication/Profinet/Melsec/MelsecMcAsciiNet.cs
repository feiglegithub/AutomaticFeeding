...
namespace NJIS.PLC.Communication.Profinet.Melsec
{
    ...
    /// <example>
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\MelsecAscii.cs"
    ...
    public class MelsecMcAsciiNet : NetworkDeviceBase<MelsecQnA3EAsciiMessage, RegularByteTransform>
    {
        ...
        {
            // 解析指令
            var command = BuildWriteCommand(address, value, NetworkNumber, NetworkStationNumber);
            ...
            // 错误码验证
            var errorCode = Convert.ToUInt16(Encoding.ASCII.GetString(read.Content, 18, 4), 16);
            if (errorCode != 0)
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\MelsecAscii.cs"
        ///         region="ReadBool" title="Bool类型示例" />
        /// </example>
        public OperateResult<bool[]> ReadBool(string address, ushort length)
        {
            // 解析地址
            var analysis = MelsecHelper.McAnalysisAddress(address);
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\MelsecAscii.cs"
        ///         region="WriteBool" title="Write示例" />
        /// </example>
        /// <returns>返回写入结果</returns>
        ...
        public static OperateResult<byte[]> BuildReadCommand(string address, ushort length, byte networkNumber = 0,
        ...
        /// <returns>带有成功标志的指令数据</returns>
        public static OperateResult<byte[]> BuildWriteCommand(string address, byte[] value, byte networkNumber = 0,
            byte networkStationNumber = 0)
        {
            var analysis = MelsecHelper.McAnalysisAddress(address);
            if (!analysis.IsSuccess) return OperateResult.CreateFailedResult<byte[]>(analysis);
            ...
            var _PLCCommand = new byte[42 + value.Length];
        ...
        public static OperateResult<byte[]> ExtractActualData(byte[] response, bool isBit)
        {
            ...
            {
                // 位读取
                var Content = new byte[response.Length - 22];
                for (var i = 22; i < response.Length; i++)
                {
            ...
            {
                // 字读取
                var Content = new byte[(response.Length - 22) / 2];
                for (var i = 0; i < Content.Length / 2; i++)
                {
                    var tmp = Convert.ToUInt16(Encoding.ASCII.GetString(response, i * 4 + 22, 4), 16);
