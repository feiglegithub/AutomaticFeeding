...
namespace NJIS.PLC.Communication.Profinet.Melsec
{
    ...
    public class MelsecA1ENet : NetworkDeviceBase<MelsecA1EBinaryMessage, RegularByteTransform>
    {
        ...
        {
            // 解析指令
            var command = BuildWriteCommand(address, value, PLCNumber);
        ...
        public override OperateResult<byte[]> Read(string address, ushort length)
        {
            ...
            // 错误代码验证
            if (read.Content[1] != 0)
                return new OperateResult<byte[]>(read.Content[1],
                    StringResources.Language.MelsecPleaseReferToManulDocument);
        ...
        public static OperateResult<byte[]> BuildReadCommand(string address, ushort length, byte plcNumber)
        {
            ...
            // 默认信息----注意：高低字节交错
            var subtitle = analysis.Content1.DataType == 0x01 ? (byte) 0x00 : (byte) 0x01;

            var _PLCCommand = new byte[12];
        ...
        public static OperateResult<byte[]> BuildWriteCommand(string address, byte[] value, byte plcNumber)
        {
            ...
            // 默认信息----注意：高低字节交错
            var subtitle = analysis.Content1.DataType == 0x01 ? (byte) 0x02 : (byte) 0x03;

            var _PLCCommand = new byte[12 + value.Length];
            _PLCCommand[0] = subtitle; // 副标题
            _PLCCommand[1] = plcNumber; // PLC号
            _PLCCommand[2] = 0x0A; // CPU监视定时器（L）这里设置为0x00,0x0A，等待CPU返回的时间为10*250ms=2.5秒
            _PLCCommand[3] = 0x00; // CPU监视定时器（H）
        ...
        public static OperateResult<byte[]> ExtractActualData(byte[] response, bool isBit)
        {
            ...
            {
                // 位读取
                var Content = new byte[(response.Length - 2) * 2];
            ...
            {
                // 字读取
                var Content = new byte[response.Length - 2];
