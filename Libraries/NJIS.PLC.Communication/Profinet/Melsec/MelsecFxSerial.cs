...
namespace NJIS.PLC.Communication.Profinet.Melsec
{
    ...
    /// <example>
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\MelsecFxSerial.cs"
    ///         region="Usage" title="简单的使用" />
    /// </example>
    public class MelsecFxSerial : SerialDeviceBase<RegularByteTransform>
    {
        ...
        public override OperateResult Write(string address, byte[] value)
        {
            ...
            // 结果验证
            var checkResult = CheckPlcWriteResponse(read.Content);
        ...
        public static OperateResult<byte[]> BuildWriteBoolPacket(string address, bool value)
        {
            ...
            var startAddress = analysis.Content2;
        ...
        public static OperateResult<byte[]> BuildReadWordCommand(string address, ushort length)
        {
            ...
            var startAddress = addressResult.Content;

            var _PLCCommand = new byte[11];
            _PLCCommand[0] = 0x02; // STX
        ...
        public static OperateResult<byte[], int> BuildReadBoolCommand(string address, ushort length)
        {
            ...
            // 计算下实际需要读取的数据长度
            var length2 = (ushort) ((addressResult.Content2 + length - 1) / 8 - addressResult.Content2 / 8 + 1);

            var startAddress = addressResult.Content1;
            var _PLCCommand = new byte[11];
            _PLCCommand[0] = 0x02; // STX
            _PLCCommand[1] = 0x30; // Read
            _PLCCommand[2] = MelsecHelper.BuildBytesFromData(startAddress)[0]; // 偏移地址
        ...
        public static OperateResult<byte[]> BuildWriteWordCommand(string address, byte[] value)
        {
            ...
            var startAddress = addressResult.Content;
            var _PLCCommand = new byte[11 + value.Length];
            _PLCCommand[0] = 0x02; // STX
        ...
        public static OperateResult<byte[]> ExtractActualData(byte[] response)
        {
            try
            {
                var data = new byte[(response.Length - 4) / 2];
        ...
        {
            var extraResult = ExtractActualData(response);
            ...
            try
            {
                ...
                var array = SoftBasic.ByteToBoolArray(extraResult.Content, extraResult.Content.Length * 8);
        ...
        private static OperateResult<ushort, ushort, ushort> FxCalculateBoolStartAddress(string address)
        {
            ...
            {
                return new OperateResult<ushort, ushort, ushort>(StringResources.Language
