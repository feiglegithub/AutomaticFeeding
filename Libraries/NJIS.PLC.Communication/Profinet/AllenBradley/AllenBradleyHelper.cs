...
namespace NJIS.PLC.Communication.Profinet.AllenBradley
{
    ...
    public class AllenBradleyHelper
    {
        ...
        {
            var buffer = new byte[commandSpecificData.Length + 24];
        ...
        public static byte[] PackRequsetRead(string address)
        {
            ...
            var offect = 0;
            var tagNames = address.Split(new[] {'.'}, StringSplitOptions.RemoveEmptyEntries);
            ...
            for (var i = 0; i < tagNames.Length; i++)
            {
                ...
                var nameBytes = Encoding.ASCII.GetBytes(tagNames[i]);
        ...
        public static byte[] PackCommandSpecificData(params byte[][] cips)
        {
            ...
            else
            {
                ...
                var offect = (ushort) (0x02 + 2 * cips.Length);
                count += 2 * cips.Length;
        ...
        public static OperateResult<byte[]> ExtractActualData(byte[] response, bool isRead)
        {
            ...
            var offect = 38;
            var count = BitConverter.ToUInt16(response, 38); // 剩余总字节长度，在剩余的字节里，有可能是一项数据，也有可能是多项
            if (BitConverter.ToInt32(response, 40) == 0x8A)
            {
                ...
                {
                    var offectStart = BitConverter.ToUInt16(response, offect + 2 + i * 2) + offect;
                    var offectEnd = i == dataCount - 1
                        ? response.Length
                        : BitConverter.ToUInt16(response, offect + 4 + i * 2) + offect;
                    var err = BitConverter.ToUInt16(response, offectStart + 2);
                    switch (err)
                    {
                        case 0x04:
                    ...
                    if (isRead)
                    {
                        for (var j = offectStart + 6; j < offectEnd; j++)
                        {
                            data.Add(response[j]);
                        }
                    }
            ...
            else
            {
                ...
                if (isRead)
                {
                    for (var i = offect + 8; i < offect + 2 + count; i++)
                    {
                        data.Add(response[i]);
                    }
                }
