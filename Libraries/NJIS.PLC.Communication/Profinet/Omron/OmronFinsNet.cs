...
namespace NJIS.PLC.Communication.Profinet.Omron
{
    ...
    /// <remarks>
    ///     <note type="important">
    ///         实例化之后，使用之前，需要初始化三个参数信息，具体见三个参数的说明：<see cref="SA1" />，<see cref="DA1" />，<see cref="DA2" />
    ///     </note>
    ///     <br />
    ...
    /// <example>
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\OmronFinsNet.cs"
    ...
    public class OmronFinsNet : NetworkDeviceBase<FinsMessage, ReverseWordTransform>
    {
        ...
        {
            // 握手信号
            var read = ReadFromCoreServerBase(socket, handSingle);
            ...
            var status = BitConverter.ToInt32(buffer, 0);
        ...
        public override OperateResult Write(string address, byte[] value)
        {
            ...
            // 数据有效性分析
            var valid = ResponseValidAnalysis(read.Content, false);
        ...
        {
            var buffer = new byte[26 + cmd.Length];
            Array.Copy(handSingle, 0, buffer, 0, 4);
            var tmp = BitConverter.GetBytes(buffer.Length - 8);
            Array.Reverse(tmp);
            tmp.CopyTo(buffer, 4);
            buffer[11] = 0x02;
        ...
        public OperateResult<byte[]> BuildWriteCommand(string address, byte[] value, bool isBit)
        {
            ...
            var _PLCCommand = new byte[8 + value.Length];
            _PLCCommand[0] = 0x01;
            _PLCCommand[1] = 0x02;
        ...
        public override OperateResult<byte[]> Read(string address, ushort length)
        {
            ...
            // 数据有效性分析
            var valid = ResponseValidAnalysis(read.Content, true);
        ...
        private static OperateResult<OmronFinsDataType, byte[]> AnalysisAddress(string address, bool isBit)
        {
            ...
            try
            {
                ...
                {
                    // 位操作
                    var splits = address.Substring(1).Split(new[] {'.'}, StringSplitOptions.RemoveEmptyEntries);
                ...
                {
                    // 字操作
                    var addr = ushort.Parse(address.Substring(1));
                    result.Content2 = new byte[3];
                    result.Content2[0] = BitConverter.GetBytes(addr)[1];
                    result.Content2[1] = BitConverter.GetBytes(addr)[0];
                }
        ...
        private static OperateResult<byte[]> ResponseValidAnalysis(byte[] response, bool isRead)
        {
            ...
            if (response.Length >= 16)
            {
                ...
                var err = BitConverter.ToInt32(buffer, 0);
                ...
                if (response.Length >= 30)
                {
                    ...
                    // 读取操作
                    var content = new byte[response.Length - 30];
