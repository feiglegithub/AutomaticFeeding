...
using NJIS.PLC.Communication.Core.Address;
...
namespace NJIS.PLC.Communication.Profinet.Siemens
{
    ...
    public class SiemensS7Net : NetworkDeviceBase<S7Message, ReverseBytesTransform>
    {
        ...
        protected override OperateResult InitializationOnConnect(Socket socket)
        {
            ...
            var read_first = ReadFromCoreServer(socket, plcHead1);
            if (!read_first.IsSuccess) return read_first;

            // 第二次握手 -> Second handshake
            var read_second = ReadFromCoreServer(socket, plcHead2);
        ...
        {
            var read = ReadFromCoreServer(plcOrderNumber);
        ...
        public byte Slot
        {
            ...
            {
                plc_slot = value;
                plcHead1[21] = (byte) (plc_rack * 0x20 + plc_slot);
            }
        }
        ...
        {
            var read = ReadFromCoreServer(S7_HOT_START);
            ...
            if (read.Content[20] != pduAlreadyStarted) return new OperateResult("Can not start PLC");
        ...
        {
            var read = ReadFromCoreServer(S7_COLD_START);
            if (!read.IsSuccess) return read;
        ...
        {
            var read = ReadFromCoreServer(S7_STOP);
            if (!read.IsSuccess) return read;

            if (read.Content.Length < 19) return new OperateResult("Receive error");

            if (read.Content[19] != pduStop) return new OperateResult("Can not stop PLC");
            if (read.Content[20] != pduAlreadyStopped) return new OperateResult("Can not stop PLC");
        ...
        public override OperateResult<byte[]> Read(string address, ushort length)
        {
            ...
            {
                var readLength = (ushort) Math.Min(length - alreadyFinished, 200);
                addressResult.Content.Length = readLength;
                var read = Read(new[] {addressResult.Content});
                if (!read.IsSuccess) return read;
        ...
        private OperateResult<byte[]> ReadBitFromPLC(string address)
        {
            ...
            {
                var buffer = new byte[receiveCount];
        ...
        ///     One-time from the PLC to obtain all the data, in order to return a unified buffer, need to be processed
        ///     sequentially, two array length must be consistent
        /// </summary>
        /// <param name="address">
        ///     起始地址，格式为I100，M100，Q100，DB20.100 ->
        ///     Starting address, formatted as I100,M100,Q100,DB20.100
        ...
        private OperateResult<byte[]> ReadS7AddressData(S7AddressData[] s7Addresses)
        {
            ...
            if (read.Content.Length >= 21 && read.Content[20] == s7Addresses.Length)
            {
                ...
                var kk = 0;
                var ll = 0;
                for (var ii = 21; ii < read.Content.Length; ii++)
                {
                    if (ii + 1 < read.Content.Length)
                    {
                        ...
                        {
                            return new OperateResult<byte[]>(read.Content[ii],
                                StringResources.Language.SiemensReadLengthOverPlcAssign);
                        }
                    }
                }
        ...
        {
            return GetBytesResultFromBytes(Read(address, length));
        }
        ...
        {
            var write = ReadFromCoreServer(entireValue);
            if (!write.IsSuccess) return write;

            if (write.Content[write.Content.Length - 1] != 0xFF)
            {
                // 写入异常 -> WriteError
                return new OperateResult(write.Content[write.Content.Length - 1],
                    StringResources.Language.SiemensWriteError + write.Content[write.Content.Length - 1]);
            }
        ...
        ///     Write a bit of PLC, for example  "M100.6",  "I100.7",  "Q100.0",  "DB20.100.0", if only write  "M100" defaults to
        ///     "M100.0"
        /// </summary>
        /// <param name="address">
        ...
        public override OperateResult Write(string address, string value)
        {
            ...
            if (CurrentPlc != SiemensPLCS.S200Smart)
            {
                ...
                return Write(address,
                    SoftBasic.SpliceTwoByteArray(new[] {readLength.Content[0], (byte) buffer.Length}, buffer));
            }

            return Write(address, SoftBasic.SpliceTwoByteArray(new[] {(byte) buffer.Length}, buffer));
        }
        ...
        public OperateResult<string> ReadString(string address,int length)
        {
        ...
        public OperateResult<string> ReadString(string address)
        {
            if (CurrentPlc != SiemensPLCS.S200Smart)
            {
                ...
                if (read.Content[0] == 0 || read.Content[0] == 255)
                    return new OperateResult<string>(
                        "Value in plc is not string type"); // max string length can't be zero
                ...
                return OperateResult.CreateSuccessResult(Encoding.ASCII.GetString(readString.Content, 2,
                    readString.Content.Length - 2));
            }
            else
            {
                ...
                return OperateResult.CreateSuccessResult(Encoding.ASCII.GetString(readString.Content, 1,
                    readString.Content.Length - 1));
            }
        }
        ...
        /// <returns>带有是否成功的字符串结果类对象</returns>
        public OperateResult<string> ReadString(string address,ushort lenght)
        {
            if (CurrentPlc != SiemensPLCS.S200Smart)
            {
                var read = Read(address, lenght);
                if (!read.IsSuccess) return OperateResult.CreateFailedResult<string>(read);
        ...
        private readonly byte[] plcOrderNumber =
        {
        ...
        public static OperateResult<byte[]> BuildReadCommand(S7AddressData[] s7Addresses)
        {
            ...
            var readCount = s7Addresses.Length;
            var _PLCCommand = new byte[19 + readCount * 12];
            // ======================================================================================
            ...
            _PLCCommand[15] =
                0x00; // 读取内部数据时为00，读取CPU型号为Data数据长度 -> Read internal data is 00, read CPU model is data length
            _PLCCommand[16] = 0x00;
            // =====================================================================================
            ...
            for (var ii = 0; ii < readCount; ii++)
            {
                ...
                // 按字为单位 -> by word
                _PLCCommand[22 + ii * 12] =
                    0x02; // (byte)(address[ii].Content1 == 0x1D ? 0x1D : address[ii].Content1 == 0x1C ? 0x1C : 0x02);
        ...
        {
            var _PLCCommand = new byte[35 + data.Length];
            _PLCCommand[0] = 0x03;
            _PLCCommand[1] = 0x00;
            // 长度 -> Length
        ...
        public static OperateResult<byte[]> BuildWriteBitCommand(string address, bool data)
        {
            ...
            var _PLCCommand = new byte[35 + buffer.Length];
            _PLCCommand[0] = 0x03;
            _PLCCommand[1] = 0x00;
            // 长度 -> length
