...
namespace NJIS.PLC.Communication.Profinet.Siemens
{
    ...
    /// <example>
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensFetchWriteNet.cs"
    ...
    public class SiemensFetchWriteNet : NetworkDeviceBase<FetchWriteMessage, ReverseBytesTransform>
    {
        ...
        public override OperateResult Write(string address, byte[] value)
        {
            ...
            var write = ReadFromCoreServer(command.Content);
            if (!write.IsSuccess) return write;

            // 错误码验证 -> Error code Verification
            if (write.Content[8] != 0x00)
                return new OperateResult(write.Content[8],
                    StringResources.Language.SiemensWriteError + write.Content[8]);
        ...
        ///     Write the bool array to the PLC, return the value description, for example, if you write M100, then data[0]
        ///     corresponds to M100.0
        /// </summary>
        /// <param name="address">起始地址，格式为M100,I100,Q100,DB1.100 -> Starting address, formatted as M100,I100,Q100,DB1.100</param>
        ...
        ///         There is a risk in bulk writing to a bool array, because it is possible to write arrays of multiples of length
        ///         8 in bulk, otherwise it will affect the data of other bits, please use sparingly.
        ///     </note>
        /// </remarks>
        ...
        public override OperateResult<byte[]> Read(string address, ushort length)
        {
            ...
            if (read.Content[8] != 0x00)
                return new OperateResult<byte[]>(read.Content[8], StringResources.Language.SiemensFWError);

            // 读取正确 -> Read Right
            var buffer = new byte[read.Content.Length - 16];
        ...
        public static OperateResult<byte[]> BuildWriteCommand(string address, byte[] data)
        {
            ...
            var _PLCCommand = new byte[16 + data.Length];
            _PLCCommand[0] = 0x53;
            _PLCCommand[1] = 0x35;
            _PLCCommand[2] = 0x10;
            _PLCCommand[3] = 0x01;
