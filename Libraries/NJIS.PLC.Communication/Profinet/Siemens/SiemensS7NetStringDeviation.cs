...
//   文 件 名：SiemensS7NetStringDeviation.cs
//   创建时间：2019-08-19 12:00
//   作    者：
//   说    明：
//   修改时间：2019-08-19 12:00
//   修 改 人：
...
namespace NJIS.PLC.Communication.Profinet.Siemens
{
    ...
    /// <example>
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
    ///         region="Usage" title="简单的短连接使用" />
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
    ///         region="Usage2" title="简单的长连接使用" />
    /// </example>
    public class SiemensS7NetStringDeviation : NetworkDeviceBase<S7Message, ReverseBytesTransformStringDeviation>
    {
        ...
        protected override OperateResult InitializationOnConnect(Socket socket)
        {
            ...
            var readFirst = ReadFromCoreServerBase(socket, plcHead1);
            if (!readFirst.IsSuccess) return readFirst;

            // 第二次握手 -> Second handshake
            var readSecond = ReadFromCoreServerBase(socket, plcHead2);
            if (!readSecond.IsSuccess) return readSecond;
        ...
        public SiemensS7NetStringDeviation(SiemensPLCS siemens)
        {
        ...
        public SiemensS7NetStringDeviation(SiemensPLCS siemens, string ipAddress)
        {
        ...
        public override OperateResult<string> ReadString(string address, ushort length)
        {
            ...
            return ByteTransformHelper.GetResultFromBytes(Read(address, length),
                m => ByteTransform.TransString(m, length, Encoding.ASCII));
            return ByteTransformHelper.GetResultFromBytes(Read(address, length),
                m => ByteTransform.TransString(m, m[deviation - 1], Encoding.ASCII));
        }
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
        ///         region="ReadExample2" title="Read示例" />
        ///     以下是读取不同类型数据的示例
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
        ///         region="ReadExample1" title="Read示例" />
        /// </example>
        public override OperateResult<byte[]> Read(string address, ushort length)
        {
            ...
            while (alreadyFinished < length)
            {
                ...
                var read = Read(new[] {addressResult}, new[] {readLength});
        ...
        {
            var addressResult = new OperateResult<byte, int, ushort>[address.Length];
            for (var i = 0; i < address.Length; i++)
            {
                var tmp = AnalysisAddress(address[i]);
        ...
        private OperateResult<byte[]> Read(OperateResult<byte, int, ushort>[] address, ushort[] length)
        {
            ...
            return new OperateResult<byte[]>
            {
                ...
                Message = StringResources.Language.SiemensDataLengthCheckFailed
            };
        }
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
        ///         region="ReadBool" title="ReadBool示例" />
        /// </example>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
        ///         region="WriteExample2" title="Write示例" />
        ///     以下是写入不同类型数据的示例
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
        ///         region="WriteExample1" title="Write示例" />
        /// </example>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Profinet\SiemensS7Net.cs"
        ///         region="WriteBool" title="WriteBool示例" />
        /// </example>
        ...
        /// </summary>
        /// <param name="address">
        ///     起始地址，例如M100，I0，Q0，DB2.100 ->
        ///     Start address, such as M100,I0,Q0,DB2.100
        /// </param>
        /// <returns>
        ///     解析数据地址，解析出地址类型，起始地址，DB块的地址 ->
        ///     Parse data address, parse out address type, start address, db block address
        ...
        public static OperateResult<byte[]> BuildReadCommand(string address, ushort length)
        {
            ...
            return BuildReadCommand(new[] {analysis}, new[] {length});
        }
        ...
        /// <param name="length">读取数据长度 -> Read Data length</param>
        /// <returns>包含结果对象的报文 -> Message containing the result object</returns>
        public static OperateResult<byte[]> BuildReadCommand(OperateResult<byte, int, ushort>[] address,
            ushort[] length)
        {
        ...
        /// <param name="data">原始的字节数据 -> Raw byte data</param>
        /// <returns>包含结果对象的报文 -> Message containing the result object</returns>
        public static OperateResult<byte[]> BuildWriteByteCommand(OperateResult<byte, int, ushort> analysis,
            byte[] data)
        {
