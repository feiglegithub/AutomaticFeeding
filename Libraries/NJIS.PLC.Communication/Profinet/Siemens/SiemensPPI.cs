...
namespace NJIS.PLC.Communication.Profinet.Siemens
{
    ...
    public class SiemensPPI : SerialDeviceBase<ReverseBytesTransform>
    {
        ...
        {
            // 解析指令
            var command = BuildReadCommand(station, address, length, false);
            if (!command.IsSuccess) return command;

            // 第一次数据交互
            var read1 = ReadBase(command.Content);
            if (!read1.IsSuccess) return read1;

            // 验证
            if (read1.Content[0] != 0xE5)
                return new OperateResult<byte[]>("PLC Receive Check Failed:" +
                                                 SoftBasic.ByteToHexString(read1.Content, ' '));

            // 第二次数据交互
            var read2 = ReadBase(executeConfirm);
            if (!read2.IsSuccess) return read2;

            // 错误码判断
            if (read2.Content.Length < 21)
                return new OperateResult<byte[]>(read2.ErrorCode,
                    "Failed: " + SoftBasic.ByteToHexString(read2.Content, ' '));
            if (read2.Content[17] != 0x00 || read2.Content[18] != 0x00)
                return new OperateResult<byte[]>(read2.Content[19],
                    GetMsgFromStatus(read2.Content[18], read2.Content[19]));
            if (read2.Content[21] != 0xFF)
                return new OperateResult<byte[]>(read2.Content[21], GetMsgFromStatus(read2.Content[21]));
        ...
        {
            // 解析指令
            var command = BuildReadCommand(station, address, length, true);
            ...
            // 验证
            if (read1.Content[0] != 0xE5)
                return new OperateResult<bool[]>("PLC Receive Check Failed:" +
                                                 SoftBasic.ByteToHexString(read1.Content, ' '));
            ...
            // 错误码判断
            if (read2.Content.Length < 21)
                return new OperateResult<bool[]>(read2.ErrorCode,
                    "Failed: " + SoftBasic.ByteToHexString(read2.Content, ' '));
            if (read2.Content[17] != 0x00 || read2.Content[18] != 0x00)
                return new OperateResult<bool[]>(read2.Content[19],
                    GetMsgFromStatus(read2.Content[18], read2.Content[19]));
            if (read2.Content[21] != 0xFF)
                return new OperateResult<bool[]>(read2.Content[21], GetMsgFromStatus(read2.Content[21]));

            // 数据提取
            var buffer = new byte[read2.Content.Length - 27];
        ...
        {
            // 解析指令
            var command = BuildWriteCommand(station, address, value);
            if (!command.IsSuccess) return command;
            ...
            // 错误码判断
            if (read2.Content.Length < 21)
                return new OperateResult(read2.ErrorCode, "Failed: " + SoftBasic.ByteToHexString(read2.Content, ' '));
            if (read2.Content[17] != 0x00 || read2.Content[18] != 0x00)
                return new OperateResult(read2.Content[19], GetMsgFromStatus(read2.Content[18], read2.Content[19]));
            if (read2.Content[21] != 0xFF)
                return new OperateResult(read2.Content[21], GetMsgFromStatus(read2.Content[21]));
            // 数据提取
        ...
        public OperateResult Start()
        {
            byte[] cmd =
            {
                0x68, 0x21, 0x21, 0x68, station, 0x00, 0x6C, 0x32, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00,
                0x28, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFD, 0x00, 0x00, 0x09, 0x50, 0x5F, 0x50, 0x52, 0x4F, 0x47,
                0x52, 0x41, 0x4D, 0xAA, 0x16
            };
            // 第一次数据交互
            var read1 = ReadBase(cmd);
            if (!read1.IsSuccess) return read1;
        ...
        public OperateResult Stop()
        {
            byte[] cmd =
            {
                0x68, 0x1D, 0x1D, 0x68, station, 0x00, 0x6C, 0x32, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00,
        ...
        private readonly byte[] executeConfirm = {0x10, 0x02, 0x00, 0x5C, 0x5E, 0x16};
        ...
        private static OperateResult<byte, int, ushort> AnalysisAddress(string address)
        {
            ...
            try
            {
                ...
                else if (address[0] == 'D' || address.Substring(0, 2) == "DB")
                {
                    ...
                    result.Content2 =
                        S7AddressData.CalculateAddressStarted(address.Substring(address.IndexOf('.') + 1));
                }
                else if (address[0] == 'V')
                {
                    result.Content1 = 0x84;
        ...
        public static OperateResult<byte[]> BuildWriteCommand(byte station, string address, byte[] values)
        {
            ...
            var _PLCCommand = new byte[37 + values.Length];
            _PLCCommand[0] = 0x68;
            _PLCCommand[1] = BitConverter.GetBytes(_PLCCommand.Length - 6)[0];
            _PLCCommand[2] = BitConverter.GetBytes(_PLCCommand.Length - 6)[0];
            _PLCCommand[3] = 0x68;
            ...
            var count = 0;
            for (var i = 4; i < _PLCCommand.Length - 2; i++)
            {
                count += _PLCCommand[i];
            }
        ...
        public static string GetMsgFromStatus(byte errorClass, byte errorCode)
        {
            ...
            if (errorClass == 0x81 && errorCode == 0x04)
            {
                return "Miscellaneous structure error in command.  Command is not supportedby CPU";
            }

            if (errorClass == 0x84 && errorCode == 0x04)
            {
                return
                    "CPU is busy processing an upload or download CPU cannot process command because of system fault condition";
            }

            if (errorClass == 0x85 && errorCode == 0x00)
            {
            ...
            if (errorClass == 0xDC && errorCode == 0x01)
            {
        ...
        public static OperateResult<byte[]> BuildWriteCommand(byte station, string address, bool[] values)
        {
            ...
            var bytesValue = SoftBasic.BoolArrayToByte(values);
            // 68 21 21 68 02 00 6C 32 01 00 00 00 00 00 0E 00 00 04 01 12 0A 10
            var _PLCCommand = new byte[37 + bytesValue.Length];
            _PLCCommand[0] = 0x68;
            _PLCCommand[1] = BitConverter.GetBytes(_PLCCommand.Length - 6)[0];
            _PLCCommand[2] = BitConverter.GetBytes(_PLCCommand.Length - 6)[0];
            _PLCCommand[3] = 0x68;
