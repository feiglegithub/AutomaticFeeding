...
namespace NJIS.PLC.Communication.Core.Thread
{
    #region 多线程同步协调类
    ...
    /// </summary>
    public sealed class NjisAsyncCoordinator
    {
        ...
        public NjisAsyncCoordinator(Action operater)
        {
            action = operater;
        }
    ...
    /// </summary>
    public sealed class NjisReadWriteLock : IDisposable
    {
 ...
 = startBit;
                    m_mask
 = unchecked((int)((1 << numBits) - 1) << startBit);
                    m_1
 = unchecked((int)1 << startBit);
                 }
                 public void Increment(ref int value) { value
 += m_1; }
                 public void Decrement(ref int value) { value
 -= m_1; }
                 public void Decrement(ref int value, int amount) { value
 -= m_1 * amount; }
              ...
              private static BitField s_state
 = new BitField(0, 3);
              private static BitField s_readersReading
 = new BitField(3, 9);
              private static BitField s_readersWaiting
 = new BitField(12, 9);
              private static BitField s_writersWaiting
 = new BitField(21, 9);
 ...
 = s_state.Set(ls, (int)newState);
    ...
    ///     <code lang="cs"
    ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\ThreadLock.cs"
    ///         region="SimpleHybirdLockExample1" title="SimpleHybirdLock示例" />
    /// </example>
    ...
    public sealed class SoftMultiTask<T>
    {
        ...
        ///     用于触发事件的混合线程锁
        /// </summary>
        private readonly SimpleHybirdLock HybirdLock = new SimpleHybirdLock();

        /// <summary>
        ///     列表数据
        /// </summary>
        private readonly T[] m_dataList;

        /// <summary>
        ///     需要操作的方法
        /// </summary>
        private readonly Func<T, bool> m_operater;

        /// <summary>
        ///     准备启动的处理数据的线程数量
        /// </summary>
        private readonly int m_threadCount = 10;
        ...
        ///     在发生错误的时候是否强制退出后续的操作
        /// </summary>
        private bool m_isQuitAfterException;

        /// <summary>
        ///     指示处理状态是否为暂停状态
        /// </summary>
        private bool m_isRunningStop;
        ...
        private void ThreadBackground()
        {
            while (true)
            {
                ...
                // 提取处理的任务
                var index = Interlocked.Decrement(ref m_opCount);
                if (index < 0)
                {
                    // 任务完成
                    break;
        ...
        public void StartOperater()
        {
            if (Interlocked.CompareExchange(ref m_runStatus, 0, 1) == 0)
            {
                ...
                {
                    var thread = new System.Threading.Thread(ThreadBackground);
        ...
        {
            get => m_isQuitAfterException;
            set => m_isQuitAfterException = value;
        }
