    ...
    public class NetworkAlienClient : NetworkServerBase
    {
        ...
        protected override void ThreadPoolLogin(object obj)
        {
            if (obj is Socket)
            {
                ...
                var check = ReceiveMessage(socket, 5000, new AlienMessage());
                ...
                var dtu = Encoding.ASCII.GetString(check.Content.ContentBytes, 0, 11).Trim();

                // 密码失败的情况
                if (!isPasswrodRight)
                {
                    var send = Send(socket, GetResponse(StatusPasswodWrong));
                    if (send.IsSuccess) socket?.Close();
                    LogNet?.WriteWarn(ToString(), "Login Password Wrong, Id:" + dtu);
                    return;
                }

                var session = new AlienSession
                {
                    DTU = dtu,
                    Socket = socket
                };
                ...
                {
                    var send = Send(socket, GetResponse(StatusLoginForbidden));
                    if (send.IsSuccess) socket?.Close();
                ...
                {
                    var send = Send(socket, GetResponse(StatusLoginRepeat));
                    if (send.IsSuccess) socket?.Close();
                ...
                {
                    var send = Send(socket, GetResponse(StatusOk));
        ...
        ///     当有服务器连接上来的时候触发
        /// </summary>
        public event Action<NetworkAlienClient, AlienSession> OnClientConnected;
        ...
        private bool IsClientOnline(AlienSession session)
        {
            ...
            for (var i = 0; i < alreadyOnline.Count; i++)
            {
        ...
        private bool IsClientPermission(AlienSession session)
        {
            ...
            {
                for (var i = 0; i < trustOnline.Count; i++)
                {
        ...
        {
            threadCheck = new System.Threading.Thread(ThreadCheckAlienClient);
        ...
        private void ThreadCheckAlienClient()
        {
            ...
            while (true)
            {
                ...
                for (var i = alreadyOnline.Count - 1; i >= 0; i--)
                {
        ...
        private readonly byte[] password; // 密码设置
        private readonly List<AlienSession> alreadyOnline; // 所有在线信息
        private readonly SimpleHybirdLock alreadyLock; // 列表的同步锁
        private List<string> trustOnline; // 禁止登录的客户端信息
        private readonly SimpleHybirdLock trustLock; // 禁止登录的锁
