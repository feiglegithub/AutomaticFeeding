...
namespace NJIS.PLC.Communication.Core.Net.NetworkBase
{
    ...
    /// </example>
    public class NetworkDoubleBase<TNetMessage, TTransform> : NetworkBase where TNetMessage : INetMessage, new()
        where TTransform : IByteTransform, new()
    {
        ...
        ///     以下的方式演示了另一种长连接的机制
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="SetPersistentConnectionExample" title="SetPersistentConnection示例" />
        /// </example>
        ...
        private bool isPersistentConn; // 是否处于长连接的状态
        private readonly SimpleHybirdLock InteractiveLock; // 一次正常的交互的互斥锁
        private bool IsSocketError; // 指示长连接的套接字是否处于错误的状态
        private bool isUseSpecifiedSocket; // 指示是否使用指定的网络套接字访问数据
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="ByteTransform" title="ByteTransform示例" />
        /// </example>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="ConnectTimeOutExample" title="ConnectTimeOut示例" />
        /// </example>
        /// <remarks>
        ///     不适用于异形模式的连接。
        /// </remarks>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="ReceiveTimeOutExample" title="ReceiveTimeOut示例" />
        /// </example>
        /// <remarks>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="IpAddressExample" title="IpAddress示例" />
        /// </example>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="Connect1" title="连接设备" />
        ///     如果想知道是否连接成功，请参照下面的代码。
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="Connect2" title="判断连接结果" />
        /// </example>
        public OperateResult ConnectServer()
        {
            ...
            var rSocket = CreateSocketAndInitialication();
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="AlienConnect1" title="连接设备" />
        ///     如果想知道是否创建成功。通常都是成功。
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="AlienConnect2" title="判断连接结果" />
        /// </example>
        /// <remarks>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="ConnectCloseExample" title="关闭连接结果" />
        /// </example>
        ...
        ///     <code lang="cs" source="NJIS.PLC.Communication_Net45\Profinet\Siemens\SiemensS7Net.cs"
        ///         region="NetworkDoubleBase Override" title="西门子重连示例" />
        /// </example>
        ...
        {
            var result = CreateSocketAndConnect(new IPEndPoint(IPAddress.Parse(ipAddress), Port), ConnectTimeOut);
            if (result.IsSuccess)
            {
                // 初始化
                var initi = InitializationOnConnect(result.Content);
                if (!initi.IsSuccess)
                {
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="ReadFromCoreServerExample1" title="ReadFromCoreServer示例" />
        /// </example>
        ...
        public OperateResult<byte[]> ReadFromCoreServer(Socket socket, byte[] send)
        {
            ...
            // 拼接结果数据
            var Content = new byte[read.Content1.Length + read.Content2.Length];
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkDoubleBase.cs"
        ///         region="ReadFromCoreServerExample2" title="ReadFromCoreServer示例" />
        /// </example>
        public OperateResult<byte[]> ReadFromCoreServer(byte[] send)
        {
            ...
            var resultSocket = GetAvailableSocket();
            ...
            var read = ReadFromCoreServer(resultSocket.Content, send);
        ...
        {
            LogNet?.WriteDebug(ToString(),
                StringResources.Language.Send + " : " + SoftBasic.ByteToHexString(send, ' '));

            var netMsg = new TNetMessage
            {
                SendBytes = send
            };
            ...
            {
                // 接收数据信息
                var resultReceive = ReceiveMessage(socket, ReceiveTimeOut, netMsg);
                if (!resultReceive.IsSuccess)
                {
                    socket?.Close();
                    return new OperateResult<byte[], byte[]>(
                        StringResources.Language.ReceiveDataTimeout + ReceiveTimeOut);
                }

                LogNet?.WriteDebug(ToString(), StringResources.Language.Receive + " : " +
                                               SoftBasic.ByteToHexString(SoftBasic.SpliceTwoByteArray(
                                                   resultReceive.Content.HeadBytes,
        ...
        /// <param name="result">原始的类型</param>
        /// <returns>转化后的类型</returns>
        protected OperateResult<byte[]> GetBytesResultFromBytes(OperateResult<byte[]> result)
        {
            return ByteTransformHelper.GetBytesResultFromBytes(result, ByteTransform);
        }
