...
namespace NJIS.PLC.Communication.Core.Net.NetworkBase
{
    ...
    public abstract class NetworkBase
    {
        ...
        /// </summary>
        /// <param name="obj">通常是 <see cref="NjisTimeOut" /> 对象 </param>
        ...
        protected OperateResult<TNetMessage> ReceiveMessage<TNetMessage>(Socket socket, int timeOut, TNetMessage netMsg)
            where TNetMessage : INetMessage
        {
            var result = new OperateResult<TNetMessage>();
            ...
            // 接收指令头
            var headResult = Receive(socket, netMsg.ProtocolHeadBytesLength);
            ...
            var contentLength = netMsg.GetContentLengthByHeadBytes();
            if (contentLength == 0)
            {
        ...
        /// </summary>
        /// <remarks>
        ///     只要实例化即可以记录日志，实例化的对象需要实现接口 <see cref="ILogNet" /> ，本组件提供了三个日志记录类，你可以实现基于 <see cref="ILogNet" />  的对象。
        /// </remarks>
        /// <example>
        ///     如下的实例化适用于所有的Network及其派生类，以下举两个例子，三菱的设备类及服务器类
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkBase.cs"
        ///         region="LogNetExample1" title="LogNet示例" />
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkBase.cs"
        ///         region="LogNetExample2" title="LogNet示例" />
        /// </example>
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkBase.cs"
        ///         region="TokenClientExample" title="Client示例" />
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkBase.cs"
        ///         region="TokenServerExample" title="Server示例" />
        /// </example>
...
//#endif
        ...
        private void SendCallBack(IAsyncResult ar)
        {
            if (ar.AsyncState is StateObject)
            {
                ...
                try
                {
                    ...
                    {
                        // 继续发送数据
                        socket.BeginSend(state.Buffer, state.AlreadyDealLength,
                            state.DataLength - state.AlreadyDealLength,
                            SocketFlags.None, SendCallBack, state);
                    }
                    else
                    {
        ...
        ///     <code lang="cs"
        ///         source="NJIS.PLC.Communication_Net45.Test\Documentation\Samples\Core\NetworkBase.cs"
        ///         region="CreateSocketAndConnectExample" title="创建连接示例" />
        /// </example>
        ...
        protected OperateResult<Socket> CreateSocketAndConnect(IPEndPoint endPoint, int timeOut)
        {
            ...
            // 超时验证的信息
            var connectTimeout = new NjisTimeOut
            {
                WorkSocket = socket,
                DelayTime = timeOut
            };
            ThreadPool.QueueUserWorkItem(ThreadPoolCheckTimeOut, connectTimeout);
        ...
        protected OperateResult<int> ReadStream(Stream stream, byte[] buffer)
        {
            ...
            var stateObject = new FileStateObject
            {
                WaitDone = WaitDone,
                Stream = stream,
            ...
            try
            {
                stream.BeginRead(buffer, 0, stateObject.DataLength, ReadStreamCallBack, stateObject);
            }
            catch (Exception ex)
            {
                LogNet?.WriteException(ToString(), ex);
        ...
        private void ReadStreamCallBack(IAsyncResult ar)
        {
            ...
            {
                var stateObject = ar.AsyncState as FileStateObject;
                try
                {
        ...
        protected OperateResult WriteStream(Stream stream, byte[] buffer)
        {
            ...
            try
            {
                stream.BeginWrite(buffer, 0, buffer.Length, WriteStreamCallBack, stateObject);
            }
            catch (Exception ex)
            {
                LogNet?.WriteException(ToString(), ex);
