...
//   文 件 名：ReverseBytesTransformStringDeviation.cs
//   创建时间：2019-08-19 12:11
//   作    者：
//   说    明：
//   修改时间：2019-08-19 12:11
//   修 改 人：
...
namespace NJIS.PLC.Communication.Core.Transfer
{
    /// <summary>
    ///     字节倒序的转换类
    ///     字符串偏移处理(字符串前两位表示最大长度与当前长度）
    /// </summary>
    public class ReverseBytesTransformStringDeviation : ByteTransformBase
    {
        ...
        public override byte[] TransByte(string value, Encoding encoding)
        {
            ...
            var index = 2;
            Array.Copy(temp, 0, datas, index, temp.Length);
            return datas;
        }
        ...
        public override string TransString(byte[] buffer, int length, Encoding encoding)
        {
            ...
            return encoding.GetString(tmp).TrimEnd('\0').Trim();
        }
