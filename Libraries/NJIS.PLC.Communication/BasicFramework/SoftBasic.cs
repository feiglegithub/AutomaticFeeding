...
namespace NJIS.PLC.Communication.BasicFramework
{
    ...
    public class SoftBasic
    {
        ...
        public static string GetSizeDescription(long size)
        {
            ...
            if (size < 1000 * 1000)
            {
                var data = (float) size / 1024;
                return data.ToString("F2") + " Kb";
            }

            if (size < 1000 * 1000 * 1000)
            {
                var data = (float) size / 1024 / 1024;
                return data.ToString("F2") + " Mb";
            }
            else
            {
                var data = (float) size / 1024 / 1024 / 1024;
        ...
        public static byte[] BytesReverseByWord(byte[] inBytes)
        {
            ...
            var buffer = ArrayExpandToLengthEven(inBytes);

            for (var i = 0; i < buffer.Length / 2; i++)
            {
                var tmp = buffer[i * 2 + 0];
        ...
        ///     Gets a string of unique random strings with a length of 20, consisting of a GUID code and a 4-digit random number
        ///     to guarantee the uniqueness of the string
        /// </summary>
        ...
        public static string CalculateStreamMD5(string data, Encoding encode)
        {
            ...
            using (MD5 md5 = new MD5CryptoServiceProvider())
            {
                var bytes_md5 = md5.ComputeHash(encode.GetBytes(data));
        ...
        ///     A common array of new methods, will automatically determine the cross-border situation, in the case of
        ///     cross-border, will be automatically truncated or filled
        /// </summary>
        ...
        public static void AddArrayData<T>(ref T[] array, T[] data, int max)
        {
            ...
            else
            {
                ...
                {
                    var tmp = new T[max];
                    for (var i = 0; i < max - data.Length; i++)
                    {
                ...
                {
                    var tmp = new T[array.Length + data.Length];
        ...
        {
            var result = new int[integer / everyLength + (integer % everyLength == 0 ? 0 : 1)];
        ...
        public static byte[] BuildAsciiBytesFrom(byte[] value)
        {
            ...
            {
                BuildAsciiBytesFrom(value[i]).CopyTo(buffer, 2 * i);
            }
        ...
        public static byte[] BoolArrayToByte(bool[] array)
        {
            ...
            var length = array.Length % 8 == 0 ? array.Length / 8 : array.Length / 8 + 1;
        ...
        {
            var buffer = new byte[Math.Min(value.Length, length)];
        ...
        public static byte[] BytesArrayRemoveDouble(byte[] value, int leftLength, int rightLength)
        {
            ...
            var buffer = new byte[value.Length - leftLength - rightLength];
