...
//  文 件 名：SqlGenerator.GetQueryProperties.cs
//  创建时间：2019-08-30 9:48
//  作    者：
//  说    明：
//  修改时间：2019-08-30 8:53
//  修 改 人：
            ...
            if (expr is MethodCallExpression methodCallExpression)
            {
                ...
                switch (methodName)
                {
                    ...
                    case "EndsWith":
                    {
                        ...
                        var propertyName = ExpressionHelper.GetPropertyNamePath(exprObj, out var isNested);
                ...
                switch (leftExpr)
                {
                            ...
                            !string.IsNullOrEmpty(rightExpr.LinkingOperator)) // AND a AND B
                        {
                            switch (rightExpr)
                            {
                                case QueryBinaryExpression rQBExpr:
                                    if (lQPExpr.LinkingOperator == rQBExpr.Nodes.Last().LinkingOperator
                                    ) // AND a AND (c AND d)
                                    {
                    ...
                    case QueryBinaryExpression lQBExpr:
                        switch (rightExpr)
                        {
                            ...
                            case QueryBinaryExpression rQBExpr:
                                if (lQBExpr.Nodes.Last().LinkingOperator == rQBExpr.LinkingOperator
                                ) // (a AND b) AND (c AND d)
                                {
                                    if (rQBExpr.LinkingOperator == rQBExpr.Nodes.Last().LinkingOperator
                                    ) // AND (c AND d)
                                    {
                                        rQBExpr.Nodes[0].LinkingOperator = rQBExpr.LinkingOperator;
