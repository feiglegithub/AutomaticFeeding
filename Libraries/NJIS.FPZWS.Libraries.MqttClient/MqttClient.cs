    ...
    public class MqttClient
    {
        ...
        public MqttClient(string serverIp, int serverPort, string clientId)
        {
            ...
            SendClient = new uPLibrary.Networking.M2Mqtt.MqttClient(serverIp, ServerPort, false, null, null,
                MqttSslProtocols.TLSv1_0);
            SerializeMode = SerializeMode.Binary;
            WaitTime = 10;
            OverTime = 12;
        }
        ...
        public void Publish(string topic, object msg, bool retain, SerializeMode mode = SerializeMode.Binary)
        {
        ...
        public void SendClientPublish(string topic, object msg, bool retain, SerializeMode mode = SerializeMode.Binary)
        {
            if (!SendClient.IsConnected) Connect();
        ...
        public void ReceiveClientPublish(string topic, object msg, bool retain,
            SerializeMode mode = SerializeMode.Binary)
        {
        ...
        internal void Response(string topic, MqttMessageBase msg, SerializeMode mode = SerializeMode.Binary)
        {
            if (!ReceiveClient.IsConnected) Connect();
            var data = msg.Serialize(mode);
            ...
            else if (msg.MessageType == MessageType.Request)
            {
                ...
                foreach (var item in _responseHandlerList)
                {
                    if (IsMatch(item.Topic, e.Topic) && item.CommandCode == msg.CommandCode)
                    {
                        ...
                        Response(topic, result, msg.SerializeMode);
                        return;
                    }
                }
            ...
            lock (_mqttLock)
            {
                if (!ReceiveClient.IsConnected)
                {
                    ...
                    if (_handlerList.Count > 0)
                    {
                        ...
                        {
                            ReceiveClient.Unsubscribe(new[] { messageHandler.Topic });
                            ReceiveClient.Subscribe(new[] { messageHandler.Topic },
                                new[] { MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE });
                        }
                    }

                    if (_responseHandlerList.Count > 0)
                    {
                        foreach (var messageHandler in _responseHandlerList)
                        {
                            ReceiveClient.Unsubscribe(new[] { messageHandler.Topic });
                            ReceiveClient.Subscribe(new[] { messageHandler.Topic },
                                new[] { MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE });
        ...
        ///     是否连接
        /// </summary>
        public bool IsConnected => ReceiveClient.IsConnected;
