...
namespace RFIDDemo
{
    public partial class Form1 : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            var str = StringToHexString(textBox8.Text);
            ...
            for (int i = 0; i < TagBuffer[comboBox3.SelectedIndex, 0] * 2; i++)
            {
                IDTemp[i] = TagBuffer[comboBox3.SelectedIndex, i + 1];
            }

            res = Reader2001.Net_EPC1G2_WriteWordBlock(m_hSocket, Convert.ToByte(EPC_Word), IDTemp, 1, Convert.ToByte(txtWStart.Text), Convert.ToByte(txtWLen.Text), mask, AccessPassWord);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                //m_hSocket = -1;
                res = Reader2001.Net_ConnectScanner(ref m_hSocket, readerip, readerport, hostip, hostport);
                if (res == 0) { break; }
            }
        ...
        {
            //Reader2001.Net_DisconnectScanner();
            Reader2001.DisconnectScanner(m_hSocket);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            res = Reader2001.Net_SetAntenna(m_hSocket, be_antenna);
            ...
            //设置天线
            res = Reader2001.Net_SetAntenna(m_hSocket, 1);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //读EPC标签
            res = Reader2001.Net_EPC1G2_ReadLabelID(m_hSocket, mem, Convert.ToByte(txtRStart.Text), Convert.ToByte(txtRLen.Text), mask, IDBuffer, ref nCounter);
            if (res == OK)
            {
                //int kk = 0;
            ...
            if (comboBox3.Text != "")
            {
                ...
                //通过标签读取托盘号
                res = Reader2001.Net_EPC1G2_ReadWordBlock(m_hSocket, Convert.ToByte(EPC_Word), IDTemp, 1, Convert.ToByte(txtRStart.Text), Convert.ToByte(txtRLen.Text), DB, AccessPassWord);
                if (res == OK)
                {
                    str = "";
                    for (i = 0; i < Convert.ToByte(txtRLen.Text) * 2; i++)
                    {
                    ...
                    if (this.txtInWareName.Text == "2001")
                    {
