...
namespace WCS
{
    public partial class DdjDemo : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //if (!OPCHelper.IsDdjCanRun(Ddj)) { return; }

            //TaskInfo task = new TaskInfo();
            //task.ddj = Ddj;
            //task.TaskId = int.Parse(this.txtTaskNo.Text);
            //task.PalletCode = this.txtPallet.Text;

            //task.FromRow = int.Parse(this.txtFromRow.Text);
            //task.FromColumn = int.Parse(this.txtFromColumn.Text);
            //task.FromLayer = int.Parse(this.txtFromLayer.Text);
            //task.ToRow = int.Parse(this.txtToRow.Text);
            //task.ToColumn = int.Parse(this.txtToColumn.Text);
            //task.ToLayer = int.Parse(this.txtToLayer.Text);

            //var rlt = OPCHelper.WriteTaskToSC(task);

            //MessageBox.Show(rlt ? "Success" : "Fault");
            //if (type == 1)
            //{
            //    //入库任务
            //    task.ToRow = int.Parse(this.txtFromRow.Text);
            //    task.ToColumn = int.Parse(this.txtFromColumn.Text);
            //    task.ToLayer = int.Parse(this.txtFromLayer.Text);
