...
namespace RFIDCA20022
{
    class Program
    {
        static void Main(string[] args)
        {
            var rip = "192.168.1.223";
            var inport = 2002;
            ...
            UdpRfider ur = new UdpRfider(rip, 8024, "192.168.1.253", 8023);
            Console.Title = $"{inport}(2)入口扫码器";
            int rows = 0;
            ...
            if (conn_rlt)
            {
                Console.WriteLine($"入口{inport}(2)扫码器{rip}：连接成功！");
                ...
                try
                {
                    while (true)
                    {
                        ...
                        if (pallet.Length == 11)
                        {
                            ...
                            Console.WriteLine($"入口{inport}(2)扫码器{rip}：读取条码{pallet}成功！时间：{DateTime.Now.ToString()}");
                            rows++;
