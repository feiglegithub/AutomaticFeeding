...
namespace NJIS.FPZWS.LineControl.Manager.Service
{
    public partial class CuttingManagerService : ServiceBase
    {
        Thread requestBoard12Thread;
        Thread requestBoard13Thread;
        Thread requestPilerThread;
        Thread inPlace10Thread;
        Thread inPlace11Thread;
        ...
        {
            ////上垛（要料）
            requestPilerThread = new Thread(new ThreadStart(requestPilerLoop));
            requestPilerThread.Start();

            //10站台到位
            inPlace10Thread = new Thread(new ThreadStart(inPlace10Loop));
            inPlace10Thread.Start();

            //11站台到位
            inPlace11Thread = new Thread(new ThreadStart(inPlace11Loop));
            inPlace11Thread.Start();

            //12推板
            requestBoard12Thread = new Thread(new ThreadStart(requestBoard12Loop));
            requestBoard12Thread.Start();
            
            //13推板
            requestBoard13Thread = new Thread(new ThreadStart(requestBoard13Loop));
            requestBoard13Thread.Start();
        ...
        {
            requestBoard12Thread.Abort();
            requestBoard13Thread.Abort();
            requestPilerThread.Abort();
            inPlace10Thread.Abort();
            inPlace11Thread.Abort();
        ...
        private void heartbeat()
        {
            while (true)
            {
                ...
                if (flag)
                {
                    string heartbeatAddr = ConfigurationSettings.AppSettings["heartbeatAddr"];
                    bool heartbeatValue = plc.ReadBool(heartbeatAddr);

                    plc.Write(heartbeatAddr, heartbeatValue == false ? true : false);
                }
        ...
        public bool requestPilerByStatus(BatchGroupPlusStatus batchGroupPlusStatus, string ResultAddr1002, string TriggerInAddr1002, int TriggerOutValue)
        {
            ...
            //    .GetBatchGroupPlusByStatusAndMinBatchIndex(batchGroupPlusStatus);
            List<BatchGroupPlus> listBatchGroupPlus = lineControlCuttingServicePlus
                .GetBatchGroupPlusByStatus(batchGroupPlusStatus);
            if (listBatchGroupPlus.Count > 0)
            {
                foreach (var batchGroupPlus in listBatchGroupPlus)
                {
                    ...
                    if (batchProductionDetails.DifferenceNumber > 0)
                    {
                            ...
                            if (requestCount > 0)
                            {
                                ...
                                if (reqFlag)
                                {
                                    ...
                                    if (listWMSTask.Count > 0)
                                    {
                                        ...
                                        int differenceNumber = batchProductionDetails.DifferenceNumber - requestCount;
                                ...
                                else
                                {
                                    ...
                                    //resultMsg.Message.Substring(0, resultMsg.Message.IndexOf("在"))
                                    //plc.Write(TriggerInAddr1002, TriggerOutValue);
                                    PublicUtils.PLCWriteInt(plc, TriggerInAddr1002, TriggerOutValue);
                                    lineControlCuttingServicePlus.InsertPLCLog(PublicUtils.newPLCLog("1002", TriggerType.LineControl, $"回馈TriggerIn：{TriggerOutValue}"
                                        , LogType.GENERAL));
                        ...
                        break;//需要上料，跳出循环，预防再次要料，不管是否要料成功
                    }
                }
        ...
        /// 10、11到位
        /// </summary>
        private void inPlace(int station)
        {
            bool plcIsConnect = plc.CheckConnect();
            if (plcIsConnect)
            {
            ...
            int TriggerOutValue)
        {
            ...
            $"回馈SumCount：{batchNamePilerNoBind.Count}", LogType.GENERAL));
                ...
                lineControlCuttingServicePlus.InsertPLCLog(PublicUtils.newPLCLog(station, TriggerType.LineControl,
                $"回馈CarryPanel：{CarryPanelForPlc}", LogType.GENERAL));
                //lineControlCuttingServicePlus.InsertPLCLog(PublicUtils.newPLCLog(station, TriggerType.LineControl,
                //$"回馈CarryPanel：{CarryPanelForPlc},WMS实际HasUpProtect={wmsTask.HasUpProtect}", LogType.GENERAL));
        ...
        /// 循环检查12请求推板
        /// </summary>
        private void requestBoard12Loop()
        {
            while (true)
            {
                try
        ...
        /// 循环检查13请求推板
        /// </summary>
        private void requestBoard13Loop()
        {
            while (true)
            {
                try
        ...
        /// 监控12、13站台推板请求
        /// </summary>
        private void requestBoard(int station)
        {
            ...
            if (plcIsConnect)
            {
                ...
                {//13号站台
                    ...
                    string PLCResultAddr13 = ConfigurationSettings.AppSettings["PLCResultAddr13"];
                    ...
                    if (triggerDifference13 == 1)//13号站台请求推板
                    {
                        ...
                        requestBoardAction(PilerNoAddr, CountAddr, "13", SawNoAddr, PushCountAddr, ResultAdrr13,
                            TriggerInAddr13, TriggerOutValue13, PLCResultAddr13);
                    }
            ...
            string PLCResultAddr)
        {
            ...
            //余料回库业务
            List<BatchGroupPlus> listBatchGroupPlus2 = lineControlCuttingServicePlus.
                GetBatchGroupPlusByBatchName(batchNamePilerNoBind.BatchName);
            ...
            List<CuttingSawFileRelationPlus> listCuttingSawFileRelationPlus2 = lineControlCuttingServicePlus.
                GetCuttingSawFileRelationPlus(batchNamePilerNoBind.StackName,CuttingSawFileRelationPlusStatus.Unassigned,
                SawType.TYPE1);
            ...
            if (listCuttingSawFileRelationPlus2.Count > 0)
            {
                ...
                feedBackRequestBoardAndUpdateDB(station, listBatchNamePilerNoBind, cuttingSawFileRelationPlus.BoardCount, SawNoAddr,
                    cuttingSawFileRelationPlus.SawFileName, PushCountAddr, ResultAddr, TriggerInAddr, TriggerOutValue, 
                    PLCResultAddr,listCuttingSawFileRelationPlus2);
            }
            else
            {
        ...
        private void PushBoardSuccess(string station, int boardCount, List<BatchNamePilerNoBind> listBatchNamePilerNoBind)
        {
            ...
            //已分配给1-5号锯的锯切图名称
            string sawFileName = PublicUtils.getSawFileName(lineControlCuttingServicePlus, batchNamePilerNoBind.BatchName);

            //已分配给6号锯的垛
            string stackNames = PublicUtils.getStackName6(lineControlCuttingServicePlus, batchNamePilerNoBind.BatchName);
