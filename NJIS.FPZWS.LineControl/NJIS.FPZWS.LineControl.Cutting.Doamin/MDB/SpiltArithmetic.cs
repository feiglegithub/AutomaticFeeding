...
namespace NJIS.FPZWS.LineControl.Cutting.Domain.MDB
{
    ...
    public class SpiltArithmetic
    {
        ...
        public static Tuple<TimeSpan,Dictionary<int,int>> Rebuild(DataSet data,List<CuttingStackList> cuttingStackList)
        {
            ...
            for (int i = 0; i < cuttingStackList.Count; i++)
            {
                int newPTN_INDEX = Convert.ToInt32(cuttingStackList[i].PatternName);
                foreach (DataRow row in tablePATTERNS.Rows)
                {
                    if (Convert.ToInt32(row[PTN_INDEX].ToString().Trim()) == newPTN_INDEX)
                    {
