...
//   文 件 名：ChainBufferDeletePartCommand.cs
...
namespace NJIS.FPZWS.LineControl.Drilling.Domain.Emqtt
{
    ...
    ///     缓存架初始化命令
    /// </summary>
    public class ChainBufferDeletePartCommand : EmqttCommandBase
    {
        private readonly ILogger _log = LogManager.GetLogger(typeof(ChainBufferDeletePartCommand).Name);
        private readonly IDrillingContract _service = ServiceLocator.Current.GetInstance<IDrillingContract>();

        public override string SendTopic => EmqttSetting.Current.PcsDeletePartChainBufferRep;

        public override string ReceiveTopic => EmqttSetting.Current.PcsDeletePartChainBufferReq;
