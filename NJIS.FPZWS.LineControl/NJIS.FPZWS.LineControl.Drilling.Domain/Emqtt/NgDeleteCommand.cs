...
namespace NJIS.FPZWS.LineControl.Drilling.Domain.Emqtt
{
    ...
    ///     缓存架初始化命令
    /// </summary>
    public class NgDeleteCommand : EmqttCommandBase
    {
        private readonly ILogger _log = LogManager.GetLogger(typeof(NgDeleteCommand).Name);
        ...
        public override string ReceiveTopic => EmqttSetting.Current.DeletePcsNgReq;
        ...
        public override void Execute(MqttMessageBase input)
        {
            ...
            try
            {
                if (!_service.DeleteNg(entity.PartId))
                {
                    SendAlarmMsg(new PcsErrorAlarmArgs($"板件{entity.PartId}删除失败"));
                    return;
                }
                SendMsg($"板件{entity.PartId}删除成功");
