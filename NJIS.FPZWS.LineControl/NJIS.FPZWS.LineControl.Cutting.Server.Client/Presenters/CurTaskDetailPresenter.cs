...
namespace NJIS.FPZWS.LineControl.Cutting.Server.Client.Presenters
{
    public class CurTaskDetailPresenter : PresenterBase
    {
        ...
        private CurTaskDetailPresenter()
        {
            ...
            BroadcastMessage.Register<Tuple<List<CuttingPattern>, DataSet, List<CuttingTaskDetail>, List<CuttingTaskDetail>>>(CurTaskListenService.BroadcastBindingData,
                this,tuple => { Send(CurTaskDetailControl.BindingDatas, tuple); });

            BroadcastMessage.Register<string>(CurTaskListenService.BroadcastBindingListenMsg,
                this, tips => { SendTipsMessage(tips); });

            BroadcastMessage.Register<SpiltMDBResult>(CurTaskListenService.BroadcastBindingCurTask,
                this, spiltMdbResult => { Send(CurTaskDetailControl.BindingCurTask, spiltMdbResult); });

            BroadcastMessage.Register<DeviceInfos>(CurTaskListenService.BroadcastBindingDeviceInfos,
                this, deviceInfos => { Send(CurTaskDetailControl.BindingDeviceInfos, deviceInfos); });

            BroadcastMessage.Register<List<DeviceInfos>>(CurTaskListenService.BroadcastBindingDeviceInfos,
                this, deviceInfos =>
                {
                    Send(CurTaskDetailControl.BindingDeviceInfos, deviceInfos);
                });
        ...
        private void GetCurTaskDetail()
        {
            while (true)
            {
                try
                {
                    ...
                    Send(CurTaskDetailControl.BindingDeviceInfos, divs);
        ...
        private void ExecuteSetDeviceStatus(bool enable)
        {
            try
            {
                ...
                var div = deviceInfos.FirstOrDefault(item => item.DeviceName == CuttingServerSettings.Current.CurrDeviceName);
                if (div == null)
                {
                    SendTipsMessage($"无法找到设备{CuttingServerSettings.Current.CurrDeviceName}");
