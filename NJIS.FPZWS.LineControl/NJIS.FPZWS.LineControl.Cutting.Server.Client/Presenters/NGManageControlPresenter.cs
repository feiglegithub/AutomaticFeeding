    ...
    public class NGManageControlPresenter:PresenterBase
    {
        ...
        private void ExecutePartReenter(object sender,string partId)
        {
            if (plcRequest > plcResponse)
            {
                ...
                while (!writeSuccess)
                {
                    ...
                    Plc.Write(CuttingSerialPortSetting.Current.BatchNameDbAddr, partDetail.BatchName, 26);
                    writeSuccess &= partDetail.BatchName == Plc.ReadString(CuttingSerialPortSetting.Current.BatchNameDbAddr, 26);
        ...
        private void Register()
        {
            ...
            Register<string>(SerialListenService.BroadcastBindingPartId, (partId) =>
            {
                ...
                var partInfos = LineControlCuttingContract.GetBatchTaskDetailsByPartId(partId);
                ...
                Model.CuttingTaskDetail partInfo = partInfos[0];
        ...
        private CuttingTaskDetail GetPartDetailByPartId(string partId)
        {
            bool isOffPart = false;
            var feedBacks = LineControlCuttingContract.GetCuttingFeedBacksByPartId(partId);
            var partDetails = feedBacks.ConvertAll(item => new CuttingTaskDetail
            {
            ...
            {
                var partInfos = LineControlCuttingContract.GetPartInfosByPartId(partId);
                if (partInfos.Count == 0)
                {
                    return null;
                }

                var partInfo = partInfos.FirstOrDefault(item => !string.IsNullOrWhiteSpace(item.BatchCode));

                partDetail = new CuttingTaskDetail
                {
                    BatchName = partInfo?.BatchCode,
                    Color = partInfo?.MaterialInformation,
                    IsOffPart = false,
                    IsNg = false,
            ...
            partDetail = partDetail ?? new CuttingTaskDetail
            {
                BatchName = "Default",
                IsOffPart = isOffPart,
                Color = "",
        ...
        private void GetPartDetail(object sender, string partId)
        {
            ...
            {
                SendTipsMessage(e.Message, sender);
                Send(BindingData, sender, (Model.CuttingTaskDetail)null);
            }
        }
    }
}
