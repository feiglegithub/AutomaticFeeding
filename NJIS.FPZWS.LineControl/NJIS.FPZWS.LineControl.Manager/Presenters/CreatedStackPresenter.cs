...
{/*
    public class CreatedStackPresenter: PresenterBase
    {
        ...
        private ILineControlCuttingContractPlus Contract =>
            _lineControlCuttingContract ?? (_lineControlCuttingContract = WcfClient.GetProxy<ILineControlCuttingContractPlus>());
        ...
        private void ExecuteSave(object sender, List<BatchStack> batchStacks)
        {
            ...
            {
                var totalBook = Patterns.FindAll(item => item.BatchName == batchStack.BatchName).Sum(item => item.BookCount);
                var tTotalBook = batchStack.PatternStacks.Sum(item => item.BookCount);
                if (totalBook != tTotalBook)
                {
                    SendTipsMessage($"批次：{batchStack.BatchName}建垛数据异常，请联系管理员!", sender);
                    Send(BindingData, sender, new List<BatchGroup>());
                    Send(BindingData, sender, new List<BatchStack>());
                    return;
                }
            }
            ...
            if (errorStack != null)
            {
                ...
                SendTipsMessage($"批次:{errorStack.BatchName}\n垛号：{stack.StackName}\n板材数超过40,不可保存！\n请手动微调整垛或联系管理员");
                return;
            }
            ...
            foreach (var group in stackLists.GroupBy(item => item.StackName))
            {
                var list = group.OrderBy(item => item.StackIndex).ToList();
                var batchName = list.First().BatchName;
                list.ForEach(item=>item.BatchName = batchName);
            }
            ...
            patterns.ForEach(item=>item.UpdatedTime=DateTime.Now);
            try
            {
                //新增备料任务
        ...
        private void ExecuteGetData(object sender, DateTime planDate)
        {
            try
            {
                ...
                var devices = Contract.GetCuttingDeviceInfos();
                this.Send(BindingData, sender, devices);
                var mdbParses = Contract.GetMdbParses();
                if (mdbParses.Exists(item => item.Status > 0 && item.Status < 99 && item.PlanDate == CurPlanDate))
                {
                    BatchInfos = new List<TmpBatchInfo>();
                    Patterns = new List<Pattern>();
                    this.Send(BindingData, sender, BatchInfos);
                    return;
                }

                var batchGroups = Contract.GetBatchGroupsByPlanDate(planDate);
                var patterns = Contract.GetPatternsByPlanDate(planDate);

                var unCreatedPatterns =
                    patterns.FindAll(item => !batchGroups.Exists(item1 => item.BatchName == item1.BatchName));

                //var stacks = Contract.GetStacksByPlanDate(planDate);
                //if (stacks.Count > 0)
                //{
                //    BatchInfos = new List<TmpBatchInfo>();
                //    Patterns = new List<Pattern>();
        ...
        /// 构建批次组
        /// </summary>
        /// <param name="minGroupId">组的最小id</param>
        /// <returns></returns>
        private List<BatchGroup> CreatedBatchGroups(int minGroupId)
        {
            ...
            var groupList = batchMainColors.GroupBy(
                ...
                g => g, new Helpers.Arithmetics.FuncEqualityComparer<TmpGroupKey>((x, y) =>
        ...
        private void ExecuteCreatedStacks(object sender, List<DeviceInfos> dis)
        {
            ...
            if (Patterns.Count == 0)
            {
                SendTipsMessage("没有可建垛数据!",sender);
                Send(BindingData, sender, new List<BatchGroup>());
                Send(BindingData, sender, new List<BatchStack>());
                return;
            }
            ...
            foreach (var batchStack in batchStacks)
            {
                ...
                {
                    var stackName = "SP"+patternStack.BatchName.Remove(0,5) + (++stackCount).ToString("000");
                    patternStack.Stack.StackName = stackName;
                });
            }

            var batchGroups = Contract.GetBatchGroupsByPlanDate(CurPlanDate);

            var maxGroupId = batchGroups.Count==0?0:batchGroups.Max(item => item.GroupId);

            var tBatchGroups = CreatedBatchGroups(maxGroupId);
