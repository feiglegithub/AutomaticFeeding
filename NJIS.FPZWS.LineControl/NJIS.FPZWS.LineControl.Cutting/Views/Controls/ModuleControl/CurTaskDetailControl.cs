    ...
    public partial class CurTaskDetailControl : UserControl, IView
    {
        ...
        private void ViewInit()
        {
            ...
            gvwPatterns.AddColumns(new List<ColumnInfo>()
            {
                ...
                new ColumnInfo(){FieldName =CuttingSetting.Current.DisplayOldPTN_INDEX? nameof(cp.PatternName):nameof(cp.NewPatternName),DataType = typeof(short),HeaderText = "锯切图", ReadOnly = true},
            ...
            {
                new ColumnInfo(){FieldName =CuttingSetting.Current.DisplayOldPTN_INDEX?nameof(ctd.OldPTN_INDEX): nameof(ctd.NewPTN_INDEX),DataType = typeof(int),HeaderText = "锯切图"},
        ...
        private void ExecuteBindingDatas(Tuple<List<CuttingPattern>, DataSet, List<CuttingTaskDetail>, List<CuttingTaskDetail>> tuple)
        {
            ...
            lbNextCuttingPattern.Text = nextTask == null ? "无" :
                (CuttingSetting.Current.DisplayOldPTN_INDEX ? nextTask.PatternName.ToString() : nextTask.NewPatternName.ToString());
            lbCurCuttingPattern.Text = curTask == null ? "无" :
                (CuttingSetting.Current.DisplayOldPTN_INDEX ? curTask.PatternName.ToString() : curTask.NewPatternName.ToString());
