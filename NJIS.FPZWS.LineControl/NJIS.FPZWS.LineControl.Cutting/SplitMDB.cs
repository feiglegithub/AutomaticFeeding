...
namespace NJIS.FPZWS.LineControl.Cutting.UI
{
    public class SplitMDB
    {
        ...
        private string MDBPath = Path.Combine(System.IO.Directory.GetCurrentDirectory(),"MDBs");
        ...
        /// <returns></returns>
        private bool CreatedMDBs(List<string> DestFileMDBNames)
        {
        ...
        public bool Spilt(List<Tuple<string,DataSet>> tuples)
        {
            if (!File.Exists(tmpMDBName))
            {
                return false;
            ...
            foreach (var tuple in tuples)
            {
                ...
                SpiltDatas.Add(new SplitMDB.SpiltDatas() { MDBName = tuple.Item1, Datas = GetDatas(tuple.Item1) });
            ...
            foreach(var item in SpiltDatas)
            {
                ...
                //foreach (DataRow row in PATTERNSTable.Rows)
                //{
                //    PartCount += Convert.ToInt32(row["QTY_RUN"]);
                //}
                //var BOARDSTable = data.Tables[BOARDS];
                //BOARDSTable.Rows[0]["QTY_USED"] = PartCount;
            }
            UpDate(SpiltDatas);
            return true;
        }
