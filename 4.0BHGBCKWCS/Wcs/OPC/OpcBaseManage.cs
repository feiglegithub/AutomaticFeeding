        ...
        public Item GetItem(int cpuNo, int itemNo)
        {
            lock (this)
            {
                return listItem.Find(
                    item => item.cpu.cpuNo == cpuNo && item.itemNo == itemNo);
            }
        }
    ...
    public class Cpu
    {
        ...
        public void Connect(ApplicationInstance ai)
        {
            try
            {
                ...
                session.ConnectionStatusUpdate += Session_ServerConnectionStatusUpdate;
                ...
                WCSSql.InsertLog($"地面线建立连接成功费时：{span.Milliseconds}", "LOG");
            }
            catch (Exception ex)
            {
                disconnect();
                WCSSql.InsertLog($"初始化Cpu失败：{ex.Message}", "ERROR");
            }
        }
        ...
        {
            if (!ReferenceEquals(session, sender)) return;
