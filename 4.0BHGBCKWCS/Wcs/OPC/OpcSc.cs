...
namespace WCS
{
    public class OpcSc
    {
        ...
        public static bool ClearTaskFinished(int SCNo)
        {
            //int itemNo = 16;
            //int GroupNo = start + SCNo;
            return OPCExecute.AsyncWrite(SCNo - 1, 29, 0);
        ...
        /// <returns></returns>
        public static bool WTaskIn(TaskInfo taskInfo, int SCNo)
        {
        ...
        /// <param name="SCNo">堆垛机操作台号</param>
        /// <returns></returns>
        public static bool WTaskIn(WMS_Task taskInfo, int SCNo)
        {
            int toRow = int.Parse(taskInfo.ToPosition.Split('.')[0]);
            int toColumn = int.Parse(taskInfo.ToPosition.Split('.')[1]);
            int toLayer = int.Parse(taskInfo.ToPosition.Split('.')[2]);
            ...
            OPCExecute.AsyncWrite(groupNo, 20, taskInfo.PilerNo);//写入货叉1工作序号（WORK NUMBER）
            OPCExecute.AsyncWrite(groupNo, 21, 1);//写入货叉1出发HS位（FROM HOME STAND NUMBER）
            OPCExecute.AsyncWrite(groupNo, 26, toRow);//写入货叉1目的“行”号
            OPCExecute.AsyncWrite(groupNo, 27, toColumn);//写入货叉1目的“列”号
            OPCExecute.AsyncWrite(groupNo, 28, toLayer);//写入货叉1目的“层”号
            OPCExecute.AsyncWrite(groupNo, 19, 1);//写入货叉1当前工作命令代码  1 入库
        ...
        /// <param name="OptNo">堆垛机操作台号</param>
        /// <returns></returns>
        public static bool WTaskOut(TaskInfo taskInfo, int SCNo)
        {
            int GroupNo = start + SCNo;
        ...
        /// <param name="OptNo">堆垛机操作台号</param>
        /// <returns></returns>
        public static bool WTaskOut(WMS_Task taskInfo, int SCNo)
        {
            int fromRow = int.Parse(taskInfo.FromPosition.Split('.')[0]);
            int fromColumn = int.Parse(taskInfo.FromPosition.Split('.')[1]);
            int fromLayer = int.Parse(taskInfo.FromPosition.Split('.')[2]);

            int groupNo = start + SCNo;

            OPCExecute.AsyncWrite(groupNo, 20, taskInfo.PilerNo);//写入货叉1工作序号（WORK NUMBER）
            OPCExecute.AsyncWrite(groupNo, 25, 2);//写入货叉1目标HS位（FROM HOME STAND NUMBER）
            OPCExecute.AsyncWrite(groupNo, 22, fromRow);//写入货叉1出发“行”号
            OPCExecute.AsyncWrite(groupNo, 23, fromColumn);//写入货叉1出发“列”号
            OPCExecute.AsyncWrite(groupNo, 24, fromLayer);//写入货叉1出发“层”号
            OPCExecute.AsyncWrite(groupNo, 19, 2);//写入货叉1当前工作命令代码  2 出库
